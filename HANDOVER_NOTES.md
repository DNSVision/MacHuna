...
**Converted rows lock with a green tick and are excluded from the next Convert.** That is the v1.6.21 no-double-conversion guarantee kept without emptying the list. **Row status persists across a list rebuild** - without that, adding a file to a finished batch resurrected every row and the next Convert collided with its own output. That bug was only found by driving a *real* conversion; the first version of the test set statuses by hand, so it proved the tick rendered while missing that only the EIF path ever called the marker.

**Layout:** log down to five lines, the list takes the height it gave up so it grows with the window. Default window was `1085x460` (pre-list) against ~695px of content; now `1120x740`, and **`usable_geometry()` grows a saved window that is too small** - the settings file outlives an update, so every existing user would otherwise have opened a window too small to use. Deliberately larger windows are left alone; screen position preserved.

**Verification for GUI work now has three drivers** in the session scratchpad (interaction, pixel alignment, and a real end-to-end conversion). They repeatedly caught things unit tests could not. **129 unit tests**, passing under pytest and unittest.

**Known limitation, recorded not scheduled:** "TGA source interlaced" is per batch, not per item, so a batch mixing interlaced and progressive TGA sequences applies the tick to both. David's view: not a real use case.

**Also this session:** the `.eaf` audio format was decoded from David's own test files (`~/Desktop/TEST WIPES/50i/EIF/`) - **128-byte header, sample count at 0x64, frame count at 0x6A, then 8-channel 16-bit 48kHz**, verified across all six files. DEVELOPMENT_NOTES and the website still say no `.eaf` was ever available, which is now wrong and needs correcting. **EIF audio is no longer blocked on hardware** - David has a Kayenne desk session booked and wants to prove everything there.

**Open:** correct the `.eaf` claims in the docs and site, then EIF audio read/write; the white-key investigation; and the hardware-gated items. Publishing is `./publish.sh --upload` and stays opt-in.
