...
**Understanding baseline:** commit `4f150ee` - 2026-09-03. Four releases this session, all around the new bespoke per-item output IDs on batch convert. v1.6.13 added the feature (per-item numbers for Kahuna SWS and Kayenne EIF, per-item 4-character names for Sony TGA, with blank/duplicate/destination-collision checks that block rather than overwrite, and the Sony one-clip-per-batch cap lifted when bespoke names are used). v1.6.14-v1.6.16 refined it from David's hands-on testing: fields blank on every entry to and exit from the mode, scrollbar beside the list, "Add to List" in the folder browser so a batch can be built from several folders (`merge_input_types()` gates what can share a batch; `open_files` no longer mutates state until a button is pressed), and unticking the checkbox as the start-over gesture that clears the selection outright (`_clear_selection()`). All verified in the unit tests (85) and by driving the real GUI. App-code currency before this session was `bdba160` (v1.6.12).

**On resuming:** run `git log --oneline 4f150ee..HEAD` and `git status -s` to see what changed since the baseline, and reconcile against the code before trusting this document. Update this block (commit + date + one line of what moved) at the end of any session that changes the repo. See "Staying current between sessions" in `CLAUDE.md`.
