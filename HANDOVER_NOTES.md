...
**Understanding baseline:** commit `5948e77` - 2026-09-03. Eight releases this session, all around the new bespoke per-item output IDs on batch convert. v1.6.13 added the feature (per-item numbers for Kahuna SWS and Kayenne EIF, per-item 4-character names for Sony TGA, with blank/duplicate/destination-collision checks that block rather than overwrite, and the Sony one-clip-per-batch cap lifted when bespoke names are used). v1.6.14-v1.6.19 refined it from David's hands-on testing: fields blank on every entry to and exit from the mode; scrollbar beside the list; "Add to List" in the folder browser so a batch can be built from several folders (`merge_input_types()` gates what can share a batch; `open_files` no longer mutates state until a button is pressed); unticking as the start-over gesture that clears the selection outright (`_clear_selection()`); and a blocked batch now marking every offending field with a per-row reason, scrolling to the first and focusing it (`_analyse_bespoke_ids()` feeds both the dialog and the marks; editing re-checks the whole batch via `_bespoke_recheck()` so a resolved duplicate clears its partner too). All verified in the unit tests (94) and by driving the real GUI. App-code currency before this session was `bdba160` (v1.6.12). **v1.6.20 is packaging, not app code:** the .app now has a real identifier (`com.dnsvision.machuna`) and stamps its version, David's own copy moved to `/Applications/MacHuna.app` with his Dock re-pointed there, and **publishing to the share folder is now a separate opt-in step** (checklist step 9). His Dock had been launching the unsynced share copy, so he ran v1.6.12 all day while dist/ moved on. **The share folder is still on v1.6.12 and stays there until he asks to publish.**
