...
**Understanding baseline:** commit `a9ab87c` - 2026-09-03. v1.6.14: two refinements to the v1.6.13 bespoke per-item output IDs after David's first hands-on test - fields now blank on every return to bespoke mode (on tick, and after a batch completes; preserved only across a selection change while the mode stays on), and the panel's scrollbar sits beside the list instead of at the window edge. v1.6.13 itself added the feature: per-item numbers for Kahuna SWS and Kayenne EIF, per-item 4-character names for Sony TGA, with blank/duplicate/destination-collision checks that block rather than overwrite, and the Sony TGA one-clip-per-batch cap lifted when bespoke names are in use. Both releases written and verified by that session, in the unit tests (79) and by driving the real GUI. App-code currency before them was `bdba160` (v1.6.12).

**On resuming:** run `git log --oneline a9ab87c..HEAD` and `git status -s` to see what changed since the baseline, and reconcile against the code before trusting this document. Update this block (commit + date + one line of what moved) at the end of any session that changes the repo. See "Staying current between sessions" in `CLAUDE.md`.
