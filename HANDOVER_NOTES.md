...
**Understanding baseline:** commit `e30acec` - 2026-09-07. One code release this session. **v1.6.21 fixes a selection-persistence bug David found in hands-on testing:** only a *bespoke* batch used to tidy up after itself, and even then it blanked the typed numbers without emptying the file list, so after any ordinary conversion the batch stayed loaded, the folder browser offered "Add to List", and already-converted files were silently carried into the next batch and written twice. A new `_finish_batch()` (beside `_clear_selection()` in `launch_gui`) now runs after **every** conversion, bespoke or sequential and after a cancel as well, clearing files, folders, input type and typed IDs, and logging the count. The auto-incrementing start number is deliberately left alone. 94 tests pass.

The rest of the session was **not** app code: a public product page for `dnsvision.tv/machuna` was designed and mocked up (source HTML lives only in the session scratchpad, not this repo - move it somewhere permanent if it is wanted), a build plan was written for a Help menu carrying update notifications and in-app contact via `machuna@dnsvision.tv`, and demo assets (`MacHuna_Wipe.mov` and a sting) were generated into `~/Desktop/MacHuna Demo Asset/` so the player can be screenshotted without using client material. None of that is built.

**Note the share folder is on v1.6.20, not v1.6.12 as this block previously said** - it was published at some point without being recorded here. It stays where it is until David asks to publish.
