...
**Understanding baseline:** commit `d197d4a` - 2026-09-07. A long session: two releases, and MacHuna's whole distribution story built from nothing.

**v1.6.21** - a finished conversion now clears the selection. Only a *bespoke* batch used to tidy up after itself, so after any ordinary conversion the batch stayed loaded and the folder browser's "Add to List" silently carried already-converted files into the next batch, writing them twice. `_finish_batch()` now runs after every conversion and after a cancel. Found by David in hands-on testing.

**v1.7.0 - the Help menu**, which MacHuna never had: bundled user manual, update notifications, Report a Problem, Suggest a Feature. The launch check reads `https://dnsvision.tv/machuna/version.json` on a daemon thread four seconds after the window appears, marshals back through `root.after`, and shows a dismissible strip only when something newer exists. **Silent when up to date, offline, timed out or malformed** - all three failure paths verified by driving the real GUI, not by reasoning. **It shells out to `/usr/bin/curl` on purpose and this must not be "tidied up" into `urllib`:** the bundled app ships Homebrew's libssl whose compiled-in OPENSSLDIR exists on this Mac and no other, so Python HTTPS would fail everywhere else, silently. Comparison is numeric; the download URL is validated as `https://` before being handed to `open`. The manual PDF is now a **build input** in `MacHuna.spec`.

**Distribution, built and live this session.** `https://dnsvision.tv/machuna` is the public download page (Cloudflare Worker `soft-glade-217b`), with the app on R2 at `downloads.dnsvision.tv` because Pages caps files at 25 MB. Page source is tracked in `website/`. **`./publish.sh --upload` does the whole publish** - builds, uploads the zip, deploys the site, verifies the live URLs - using a token at `~/.machuna_publish_token` scoped to R2 + Workers only (cannot touch DNS, the domain or email; expires 2027-09-07). Publishing stays **opt-in**: building never publishes. **v1.7.0 is published and live.** The `~/Desktop/Machuna Share` iCloud folder is retired and stale at v1.6.20 - ignore it.

**Things that bit us, recorded so they do not again:** the page announces the version in three places and drifted from `version.json` (publish.sh now refuses if the page is stale); a deploy check four seconds after upload reports a false failure because Cloudflare's edge is still serving the old page (now retries for a minute); and an R2 object needs `Content-Disposition: attachment` set **on first upload**, or browsers cache a headerless response and the download opens a blank page.

**Open work:** the white-key investigation is the only code item left, and it needs a K-Watch reference file. Everything else is hardware-gated (EIF on a live Kayenne, Sony MVS, P->I field order on a 1080i Kahuna). Low priority: a player screenshot for the page using `~/Desktop/MacHuna Demo Asset/MacHuna_Wipe.mov`, and retiring the iCloud folder once people have the new link.
