...
**Understanding baseline:** commit `82b72e0` - 2026-09-08. **v1.7.1 is built, released, published and live.**

**v1.7.1 - honesty about what has and has not been proven.** The `UNCONFIRMED` notes lived only in docstrings, the README and the manual, none of which is in front of someone converting a clip at 2am. The log now states it at the head of any batch to **Kayenne EIF, Kayenne TGA, Kayenne MOV or Sony MVS TGA**, and asks for a test report. **Kahuna SWS deliberately says nothing** - it is confirmed on a live Kahuna across all seven standards, and warning about it would be false caution that trains people to ignore the notices that matter.

**The bigger catch, and David's, was EIF audio.** `has_audio` is always false because Kayenne keeps clip audio in a companion `.eaf` file whose format has never been worked out, so a clip with sound converted to a *silent* EIF without a word - while SWS→SWS already warned about dropping audio. It now warns, and only when the selection actually contains audio. **This is the one thing known not to work**, as opposed to merely unproven, and the code keeps the two ideas in separate lists (`UNVERIFIED_OUTPUT_NOTES` vs `MISSING_FEATURE_NOTES`) with a test asserting they never merge. Another test pins the unverified list to the documented unknowns, so **confirming a path on hardware will fail the suite until the stale notice is removed** - that is deliberate.

**The download page gained a Verification status table** (confirmed / untested on a desk / not built yet / not available), with the EIF audio gap in its own right and an explicit ask for a real `.eaf` file - the single thing that would unblock it. The withdrawn 720p and absent SD/sF/2160p standards are framed as **invitations, not policy**: they need a reference file and someone who needs them. A **Contact developer** button sits beside Download and User manual.

**On labelling MacHuna "beta": considered and rejected, with David's agreement.** The core SWS path is hardware-confirmed and in daily use; "beta" in broadcast means *do not put this on air*, so a blanket label would be less accurate, not more cautious, and would flatten a proven path and an unproven one into the same mush. Per-output disclosure does the job properly. **Do not revisit this without new reasoning.** "Vibecoded by one person" appears once, in the contact section only - it was tried in the verification section and pulled, because it undercut the trust argument at the moment it was being made.

**Distribution is fully automated.** `./publish.sh --upload` builds, uploads the zip, deploys the site, verifies the live URLs (retrying for a minute - the edge serves the old page for a few seconds after a deploy). Token `~/.machuna_publish_token`, scoped R2 + Workers only, **expires 2027-09-07**. Publishing stays opt-in. The bucket holds only the current release; deleting the superseded zip is a manual decision and 1.7.0 has been removed.

**Open work:** the white-key investigation is the only code item, and needs a K-Watch reference. Everything else is hardware-gated - EIF on a live Kayenne (a real `.eaf` would also unblock EIF audio), Sony MVS, and P→I field order on a 1080i Kahuna. Low priority: a player screenshot for the page, and retiring the `~/Desktop/Machuna Share` iCloud folder, which David is keeping for now until everyone has the new link.
