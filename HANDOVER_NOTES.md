...
**Understanding baseline:** commit `f29a2e5` - 2026-09-07. Two releases this session plus the whole distribution story.

**v1.6.21** fixed a selection-persistence bug David found in testing: only a bespoke batch used to tidy up after itself, so after any ordinary conversion the batch stayed loaded and "Add to List" silently carried already-converted files into the next batch. `_finish_batch()` now runs after every conversion and after a cancel.

**v1.7.0 adds the Help menu** (MacHuna never had one): bundled manual, update notifications, and Report a Problem / Suggest a Feature. The launch check reads `https://dnsvision.tv/machuna/version.json` on a daemon thread, marshals back via `root.after`, and shows a dismissible strip above the destination row only when a newer version exists - silent when up to date, offline, timed out or malformed (all three failure paths verified by driving the real GUI). **It shells out to `/usr/bin/curl` deliberately**: the bundled app ships Homebrew's libssl whose compiled-in OPENSSLDIR exists on no other Mac, so Python's own HTTPS would fail silently everywhere except here. Never replace that with `urllib`, and never disable verification. Version comparison is numeric (`1.7.0` beats `1.6.21`, which string comparison gets wrong) and the download URL is validated as `https://` before being handed to `open`. The manual is now a build input (`MacHuna.spec` datas). 115 tests, passing under pytest and unittest.

**Distribution is solved and live.** `https://dnsvision.tv/machuna` is the public download page (Cloudflare Pages, project `soft-glade-217b`), with the 34 MB app on R2 at `downloads.dnsvision.tv` because Pages caps files at 25 MB. Page source is tracked in `website/`; `./publish.sh` builds `publish/` with the zip and the upload-ready `site/` folder and refuses to run if `machuna.py`, the built `.app` and `version.json` disagree on the version. **Publishing is still a manual drag into Cloudflare and still opt-in** - the site currently advertises **v1.6.21**, and stays there until David asks.

**Not done:** publishing v1.7.0; automating the Cloudflare upload (offered, needs a narrowly scoped API token); a player screenshot for the page using `~/Desktop/MacHuna Demo Asset/MacHuna_Wipe.mov`; and the white-key investigation, still the only open code item on the roadmap.
