...
- **Batches can be built up from several folders** (v1.6.15) — the folder browser's **Add to List** button appends to the current selection instead of replacing it, so a single batch can draw files from anywhere. **Select** still replaces
...
- **Data offset read from the header, not assumed** (v1.9.1) -- the video planes do not always start at byte 512; some K-Watch files declare 3072 at `0x19C`. Reading those from 512 sheared every frame by half a v210 line, which looked like the picture was split down the middle. Affected the player and all four extraction paths
- **Split files play in the Video Player** (v1.9.0) -- a clip over 4GB is a folder of 2GB chunks; select the folder or any chunk inside it and the whole clip loads. The player now decodes a batch of frames at a time rather than reading the entire video plane in one call, so a 1000-frame clip peaks at 1.4GB instead of needing a 5.5GB single read
- Batch convert with file picker, auto-incrementing file numbers, and Cancel button for stopping mid-batch
- **Converted rows lock** (v1.8.0) — a finished row shows `✓ done` (or `✗` and the reason) and Convert only ever runs rows without a status, so nothing is converted twice. The list stays on screen as the record of the session; **✕** removes a row and **Clear All** empties it. Adding files to a finished batch continues the numbering rather than starting again
- **The item list** (v1.8.0) — every selected item gets a row showing what it will be called *before* anything is written (`alpha.mov → 42.SWS`). Type a number or 4-character name per row, or tick **Number sequentially** to fill them in from the first row. Read-only rows for outputs named from the source. Blank, duplicate and already-in-the-destination values block the batch with the offending rows marked; nothing is ever overwritten. This replaced the Start number and Start slot boxes, and "Use source file number", all of which numbered files invisibly
