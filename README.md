...
- **Every conversion starts from an empty list** (v1.6.21) — when a batch finishes (or is cancelled) MacHuna clears the selection, folders and any bespoke IDs, so already-converted files can never be carried into the next batch by **Add to List** and written twice. The auto-incrementing Start number is not reset
